...
async function loadContent(section) {
  try {
    const response = await fetch(
      `tables/site_content?search=${section}&limit=100`
    );
    const result = await response.json();

    if (result.data) {
      contentData[section] = result.data.filter(
...
function renderContent(section) {
  ...
  if (items.length === 0) {
    contentBody.innerHTML =
      '<div class="empty-state"><h3>컨텐츠가 없습니다</h3></div>';
    return;
  }
        ...
        <div class="form-section">
            ...
            ${title
              .map(
                ...
                <div class="form-group">
                    <label for="${item.id}">${getFieldLabel(item.key)}</label>
                    <input type="text" id="${item.id}" data-id="${
                  item.id
            ...
            <div class="form-section">
                ...
                ${cardItems
                  .map(
                    (item) => `
                    <div class="form-group">
                        <label for="${item.id}">${getFieldLabel(
                      item.key
                    )}</label>
                        ${
...
function renderInnovationForm(items) {
  const header = items.filter(
    (i) => i.key === "title" || i.key === "description"
  );
...
document.getElementById("btnPreview").addEventListener("click", () => {
  ...
  for (const input of inputs) {
    ...
    if (id && key && value !== undefined) {
      // Use currentSection for all inputs in the current form
      if (!previewData[currentSection]) {
        previewData[currentSection] = {};
      }
      previewData[currentSection][key] = value;
    }
  }
